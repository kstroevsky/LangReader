...
MANIFEST = ARCHIVE / "development-confirmation-reservation-v12.json"
...
SOURCE_LOCK = ROOT / "docs/plans/vocabulary-validation-boundary/implementation-evidence/lexical-reconciliation-generator-source-lock-v12.json"
SOURCE_REVISION = "458f2ad80654a6d0da61cf84b0ab45fdcc9067a9"
RESERVATION_SHA256 = "68ff70eb2dd9d2ad5b35a00c50e9db160db4cf02cc1f3e8c8a77cb2ef60cc1e0"
SOURCE_LOCK_SHA256 = "8f77942d3c0caf9b26a7597285a9d4448419b280a5c6a4b7284d293595fa3acd"
...
def derived_run(root: bytes, ordinal: int) -> tuple[bytes, int, str]:
    digest = hashlib.sha256(root + f"development-confirmation-v12/run/{ordinal}".encode()).digest()
...
def validate_source_lock(lock: dict, *, current_inputs: dict | None = None, historical_inputs: dict | None = None) -> None:
    if lock.get("schemaVersion") != 1 or lock.get("lockRole") != "lexical-reconciliation-generator-provenance-v12":
        raise ValueError("unsupported v12 generator source lock")
    if lock.get("sourceRevision") != SOURCE_REVISION:
        raise ValueError("v12 generator revision changed")
    if lock.get("generatorInputsCleanAtLock") is not True or lock.get("candidateFrozen") is not False:
        raise ValueError("v12 lock has invalid clean-input/candidate status")
    if lock.get("swiftMode") != "Swift 6" or lock.get("standaloneSwiftFlags") != ["-swift-version", "6", "-warnings-as-errors", "-O"]:
        raise ValueError("v12 Swift build contract changed")
    ...
    if lock.get("generatorInputs") != historical:
        raise ValueError("v12 source lock does not match its generator commit")
    if lock.get("generatorInputs") != current:
        raise ValueError("current generator inputs no longer match active v12 reservation")
    ...
    if analysis.get("status") != "provenanceOnlyNotFrozen" or analysis.get("candidateAnalysisAndDecisionRulesChecksum") is not None:
        raise ValueError("v12 lock incorrectly claims an analysis freeze")
    if analysis.get("paths") != [
        ...
        raise ValueError("v12 analysis provenance paths changed")
    if lock.get("otherWordPOSDegradationCheckSHA256") != DEVELOPMENT_CHECK_SHA256 or sha256(DEVELOPMENT_CHECK.read_bytes()) != DEVELOPMENT_CHECK_SHA256:
        raise ValueError("v12 OtherWord POS evidence changed")
    if lock.get("protectedOutcomesAccessed") is not False:
        raise ValueError("v12 lock cannot claim protected outcome access")


def validate(manifest: dict, *, verify_files: bool = True) -> dict:
    if manifest.get("schemaVersion") != 12 or manifest.get("reservationID") != "development-confirmation-v12":
        raise ValueError("unsupported v12 development-confirmation reservation")
    if manifest.get("dataRole") != "development-confirmation" or manifest.get("executionStatus") != "reservedNotExecuted":
        raise ValueError("v12 reservation role/status changed")
    if manifest.get("reservationSourceRevision") != SOURCE_REVISION or manifest.get("supersedesAsActiveReservation") != "development-confirmation-v11":
        raise ValueError("v12 source revision or active-reservation transition changed")
    if manifest.get("historicalReservationStatuses") != {
        f"development-confirmation-v{version}": "reservedNotExecuted" for version in range(1, 12)
    }:
        raise ValueError("v12 historical reservation statuses changed")
    ...
    if not isinstance(root, str) or not re.fullmatch(r"[0-9a-f]{64}", root):
        raise ValueError("v12 seed root must be 32 lowercase-hex bytes")
    if derivation.get("algorithm") != "SHA256(rootBytes || UTF8('development-confirmation-v12/run/' || zeroBasedRunOrdinal)); seed=first8BytesUInt64BE, zero maps to one" or derivation.get("documentIdentityAlgorithm") != "first24Hex(SHA256(fullRunDigest || UTF8('/document/' || zeroBasedDocumentOrdinal)))":
        raise ValueError("v12 identity derivation changed")
    ...
    if workload.get("readersPerScenario") != 64 or workload.get("documentsPerScenario") != 8 or workload.get("lemmasPerDocument") != 400 or workload.get("scenarios") != ["well-specified", "item-residual", "response-noise", "idiosyncratic-knowledge"] or workload.get("modes") != ["all-unknown", "coverage-98"] or workload.get("warmAndCold") is not True:
        raise ValueError("v12 workload changed")

    runs = manifest.get("runs")
    if not isinstance(runs, list) or len(runs) != 3:
        raise ValueError("v12 must contain exactly three runs")
    seeds: set[int] = set()
    ...
    for ordinal, run in enumerate(runs):
        ...
        if run.get("runOrdinal") != ordinal or run.get("runID") != run_id or run.get("seed") != seed:
            raise ValueError(f"v12 run {ordinal} identity changed")
        if run.get("opaqueDocumentDerivationIDs") != expected_documents:
            raise ValueError(f"v12 run {ordinal} document identities changed")
        seeds.add(seed)
        documents.update(expected_documents)
    if len(seeds) != 3 or len(documents) != 24:
        raise ValueError("duplicate v12 identity")
    ...
    if file_count < 100 or not {1, 2, 7, 17, 20260909, 20260912, 20260913, 20260914}.issubset(prior_seeds):
        raise ValueError("v12 prior identity corpus is incomplete")
    ...
    if generator.get("path") != SOURCE_LOCK.relative_to(ROOT).as_posix() or generator.get("sha256") != SOURCE_LOCK_SHA256:
        raise ValueError("v12 generator lock binding changed")
    if verify_files:
        lock_bytes = SOURCE_LOCK.read_bytes()
        if sha256(lock_bytes) != SOURCE_LOCK_SHA256:
            raise ValueError("v12 generator lock bytes changed")
    ...
    if any(freeze.get(key) is not None for key in ("candidateFreeze", "analysisFreeze", "decisionRulesChecksum", "consumedByCandidateCycle")):
        raise ValueError("v12 prematurely claims a candidate/analysis freeze or consumption")
    if freeze.get("outcomeArtifacts") != [] or freeze.get("outcomesInspected") is not False or freeze.get("releaseHoldoutAccessAllowed") is not False:
        raise ValueError("v12 contains or authorizes protected outcomes")
    if freeze.get("requiredBeforeExecution") != ["candidate commit and clean-tree hash", "model/configuration/resource hashes", "analysis and decision-rule checksum", "reviewed decision to consume this specific reservation"]:
        raise ValueError("v12 execution prerequisites changed")
...
def self_test() -> None:
    ...
    for name, path in HISTORICAL_MANIFESTS.items():
        ...
        if not historical_seeds.issubset(prior_seeds) or not historical_documents.issubset(prior_documents):
            raise AssertionError(f"v12 prior identity corpus omitted {name} identities")
    ...
    for seed_set, document_set in ((prior_seeds | {first_seed}, prior_documents), (prior_seeds, prior_documents | {first_document})):
        try:
            ...
            raise AssertionError("accepted a synthetic v12 prior-identity collision")
    ...
    for label, mutate in mutations:
        ...
        try:
            ...
            continue
        raise AssertionError(f"accepted changed v12 {label}")
    ...
    try:
        ...
        raise AssertionError("accepted mismatched current v12 generator inputs")
    ...
    try:
        ...
        raise AssertionError("accepted mismatched historical v12 generator inputs")

    real_read_bytes = Path.read_bytes
    def missing_lock(path: Path) -> bytes:
        if path == SOURCE_LOCK:
            raise FileNotFoundError("synthetic missing v12 source lock")
    ...
    with patch.object(Path, "read_bytes", missing_lock):
        try:
            ...
            raise AssertionError("accepted missing v12 source lock")
    print("lexical-reconciliation development-confirmation reservation v12 self-test passed")
...
def main() -> None:
    ...
    if sha256(MANIFEST.read_bytes()) != RESERVATION_SHA256:
        raise ValueError("v12 reservation is missing or differs from its frozen checksum")
    if args.self_test:
        self_test()
    else:
