...
BUILD_DIR="${LEAFREADER_VOCABULARY_PARTITION_BUILD_DIR:-$ROOT_DIR/.build/vocabulary-partition-evaluator}"
EXECUTABLE="$BUILD_DIR/evaluate-vocabulary-lexical-partitions"
...
if [[ ! -x "$EXECUTABLE" ]] \
  || [[ "$ROOT_DIR/scripts/evaluate_vocabulary_lexical_partitions.swift" -nt "$EXECUTABLE" ]] \
  ...
  swiftc -O -warnings-as-errors -swift-version 6 -parse-as-library -package-name LeafReader \
    ...
    "$ROOT_DIR/scripts/evaluate_vocabulary_lexical_partitions.swift" -o "$EXECUTABLE"
fi
"$EXECUTABLE" "$@"
