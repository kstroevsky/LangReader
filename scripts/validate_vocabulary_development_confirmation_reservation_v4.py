#!/usr/bin/env python3
"""Validate the sealed, unexecuted German-portability confirmation reservation."""
...
MANIFEST = ARCHIVE / "development-confirmation-reservation-v4.json"
...
SOURCE_LOCK = ROOT / "docs/plans/vocabulary-validation-boundary/implementation-evidence/german-portability-generator-source-lock-v4.json"
SOURCE_REVISION = "3b1aa9c07659325c86e78f46f06c7c927f340e37"
RESERVATION_SHA256 = "4717a4b2b7f2b0e55d03321a5754a52ceb31c2e7b8ac232e67464fe8a89c9006"
SOURCE_LOCK_SHA256 = "1e9e67a5dd6b1aac4394959e3cbf5e4586018c9244a225b3cd6ae5a44d122a25"
DEVELOPMENT_CHECK = ARCHIVE / "german-form-label-portability-development-check-2026-09-21.json"
DEVELOPMENT_CHECK_SHA256 = "10c6a4245582b5ae381e470d91ff73acfc05138bd7bb8098410a0b99a5aa3f01"
...
def derived_run(root: bytes, ordinal: int) -> tuple[bytes, int, str]:
    digest = hashlib.sha256(root + f"development-confirmation-v4/run/{ordinal}".encode()).digest()
...
def validate_source_lock(
    ...
    *,
    ...
    if lock.get("schemaVersion") != 1 \
            or lock.get("lockRole") != "german-portability-generator-provenance-v4":
        raise ValueError("unsupported v4 generator source lock")
    if lock.get("sourceRevision") != SOURCE_REVISION:
        raise ValueError("v4 generator revision changed")
    if lock.get("generatorInputsCleanAtLock") is not True or lock.get("candidateFrozen") is not False:
        raise ValueError("v4 lock has invalid clean-input/candidate status")
    if lock.get("swiftMode") != "Swift 6" \
            or lock.get("standaloneSwiftFlags") != ["-swift-version", "6", "-warnings-as-errors", "-O"]:
        raise ValueError("v4 Swift build contract changed")
    ...
    if lock.get("generatorInputs") != historical:
        raise ValueError("v4 source lock does not match its generator commit")
    if lock.get("generatorInputs") != current:
        raise ValueError("current generator inputs no longer match active v4 reservation")
    ...
    if analysis.get("status") != "provenanceOnlyNotFrozen" \
        ...
        raise ValueError("v4 lock incorrectly claims an analysis freeze")
    if analysis.get("paths") != [
        ...
        raise ValueError("v4 analysis provenance paths changed")
    if lock.get("germanFormLabelPortabilityDevelopmentCheckSHA256") != DEVELOPMENT_CHECK_SHA256 \
            or sha256(DEVELOPMENT_CHECK.read_bytes()) != DEVELOPMENT_CHECK_SHA256:
        raise ValueError("v4 German portability development evidence changed")
    if lock.get("protectedOutcomesAccessed") is not False:
        raise ValueError("v4 lock cannot claim protected outcome access")


def validate(manifest: dict, *, verify_files: bool = True) -> dict:
    if manifest.get("schemaVersion") != 4 or manifest.get("reservationID") != "development-confirmation-v4":
        raise ValueError("unsupported v4 development-confirmation reservation")
    if manifest.get("dataRole") != "development-confirmation" \
            or manifest.get("executionStatus") != "reservedNotExecuted":
        raise ValueError("v4 reservation role/status changed")
    if manifest.get("reservationSourceRevision") != SOURCE_REVISION \
            or manifest.get("supersedesAsActiveReservation") != "development-confirmation-v3":
        raise ValueError("v4 source revision or active-reservation transition changed")
    if manifest.get("historicalReservationStatuses") != {
        ...
        raise ValueError("v4 historical reservation statuses changed")
    ...
    if not isinstance(root, str) or not re.fullmatch(r"[0-9a-f]{64}", root):
        raise ValueError("v4 seed root must be 32 lowercase-hex bytes")
    if derivation.get("algorithm") != "SHA256(rootBytes || UTF8('development-confirmation-v4/run/' || zeroBasedRunOrdinal)); seed=first8BytesUInt64BE, zero maps to one" \
            or derivation.get("documentIdentityAlgorithm") != "first24Hex(SHA256(fullRunDigest || UTF8('/document/' || zeroBasedDocumentOrdinal)))":
        raise ValueError("v4 identity derivation changed")
    ...
    if workload.get("readersPerScenario") != 64 or workload.get("documentsPerScenario") != 8 \
        ...
        raise ValueError("v4 workload changed")

    runs = manifest.get("runs")
    if not isinstance(runs, list) or len(runs) != 3:
        raise ValueError("v4 must contain exactly three runs")
    seeds: set[int] = set()
    ...
    for ordinal, run in enumerate(runs):
        ...
        if run.get("runOrdinal") != ordinal or run.get("runID") != run_id or run.get("seed") != seed:
            raise ValueError(f"v4 run {ordinal} identity changed")
        if run.get("opaqueDocumentDerivationIDs") != expected_documents:
            raise ValueError(f"v4 run {ordinal} document identities changed")
        seeds.add(seed)
        documents.update(expected_documents)
    if len(seeds) != 3 or len(documents) != 24:
        raise ValueError("duplicate v4 identity")
    ...
    if non_overlap.get("identityCorpusRevision") != SOURCE_REVISION \
            ...
            or "v1-v3" not in str(non_overlap.get("opaqueDocumentNamespaceNote", "")):
        raise ValueError("v4 non-overlap contract changed")
    ...
    if file_count < 100 or not {1, 2, 7, 17, 20260909, 20260912, 20260913, 20260914}.issubset(prior_seeds):
        raise ValueError("v4 prior identity corpus is incomplete")
    ...
    if generator.get("path") != SOURCE_LOCK.relative_to(ROOT).as_posix() \
        ...
        raise ValueError("v4 generator lock binding changed")
    if verify_files:
        lock_bytes = SOURCE_LOCK.read_bytes()
        if sha256(lock_bytes) != SOURCE_LOCK_SHA256:
            raise ValueError("v4 generator lock bytes changed")
    ...
    if any(freeze.get(key) is not None for key in (
        ...
        raise ValueError("v4 prematurely claims a candidate/analysis freeze or consumption")
    if freeze.get("outcomeArtifacts") != [] or freeze.get("outcomesInspected") is not False \
        ...
        raise ValueError("v4 contains or authorizes protected outcomes")
    if freeze.get("requiredBeforeExecution") != [
        ...
        raise ValueError("v4 execution prerequisites changed")
    return {
...
def self_test() -> None:
    ...
    for name, path in HISTORICAL_MANIFESTS.items():
        ...
        if not historical_seeds.issubset(prior_seeds) \
            ...
            raise AssertionError(f"v4 prior identity corpus omitted {name} identities")
    ...
    for seed_set, document_set in (
        ...
        try:
            ...
            raise AssertionError("accepted a synthetic v4 prior-identity collision")
    ...
    for label, mutate in mutations:
        ...
        try:
            ...
            continue
        raise AssertionError(f"accepted changed v4 {label}")
    ...
    try:
        ...
        raise AssertionError("accepted mismatched current v4 generator inputs")
    ...
    try:
        ...
        raise AssertionError("accepted mismatched historical v4 generator inputs")

    real_read_bytes = Path.read_bytes

    def missing_lock(path: Path) -> bytes:
        if path == SOURCE_LOCK:
            raise FileNotFoundError("synthetic missing v4 source lock")
    ...
    with patch.object(Path, "read_bytes", missing_lock):
        try:
            ...
            raise AssertionError("accepted missing v4 source lock")
    print("German-portability development-confirmation reservation v4 self-test passed")
...
def main() -> None:
    ...
    if sha256(MANIFEST.read_bytes()) != RESERVATION_SHA256:
        raise ValueError("v4 reservation is missing or differs from its frozen checksum")
    if args.self_test:
        self_test()
    else:
