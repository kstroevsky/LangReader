#!/usr/bin/env python3
"""Validate the sealed, unexecuted OtherWord POS confirmation reservation."""
...
MANIFEST = ARCHIVE / "development-confirmation-reservation-v6.json"
...
SOURCE_LOCK = ROOT / "docs/plans/vocabulary-validation-boundary/implementation-evidence/otherword-pos-generator-source-lock-v6.json"
SOURCE_REVISION = "4e2bd6c4f6fe0fa408d9d17d7743d5d990487207"
RESERVATION_SHA256 = "15ec2bcb8baeafc36e7983c48e4335bfb46d914d6be96c8f9875e33be52f3517"
SOURCE_LOCK_SHA256 = "45a9dd0a1fac1e0b477c8a3a4c8b7d33131fa2ac1c398e4bac3da701d8f22e49"
...
def derived_run(root: bytes, ordinal: int) -> tuple[bytes, int, str]:
    digest = hashlib.sha256(root + f"development-confirmation-v6/run/{ordinal}".encode()).digest()
...
def validate_source_lock(lock: dict, *, current_inputs: dict | None = None, historical_inputs: dict | None = None) -> None:
    ...
    if lock.get("sourceRevision") != SOURCE_REVISION:
        raise ValueError("v6 generator revision changed")
    ...
    if analysis.get("paths") != [
        ...
        raise ValueError("v6 analysis provenance paths changed")
    if lock.get("otherWordPOSDegradationCheckSHA256") != DEVELOPMENT_CHECK_SHA256 or sha256(DEVELOPMENT_CHECK.read_bytes()) != DEVELOPMENT_CHECK_SHA256:
        raise ValueError("v6 OtherWord POS evidence changed")
    if lock.get("protectedOutcomesAccessed") is not False:
        raise ValueError("v6 lock cannot claim protected outcome access")


def validate(manifest: dict, *, verify_files: bool = True) -> dict:
    if manifest.get("schemaVersion") != 6 or manifest.get("reservationID") != "development-confirmation-v6":
        raise ValueError("unsupported v6 development-confirmation reservation")
    if manifest.get("dataRole") != "development-confirmation" or manifest.get("executionStatus") != "reservedNotExecuted":
        raise ValueError("v6 reservation role/status changed")
    if manifest.get("reservationSourceRevision") != SOURCE_REVISION or manifest.get("supersedesAsActiveReservation") != "development-confirmation-v5":
        raise ValueError("v6 source revision or active-reservation transition changed")
    if manifest.get("historicalReservationStatuses") != {
        f"development-confirmation-v{version}": "reservedNotExecuted" for version in range(1, 6)
    }:
        raise ValueError("v6 historical reservation statuses changed")
    ...
    if not isinstance(root, str) or not re.fullmatch(r"[0-9a-f]{64}", root):
        raise ValueError("v6 seed root must be 32 lowercase-hex bytes")
    if derivation.get("algorithm") != "SHA256(rootBytes || UTF8('development-confirmation-v6/run/' || zeroBasedRunOrdinal)); seed=first8BytesUInt64BE, zero maps to one" or derivation.get("documentIdentityAlgorithm") != "first24Hex(SHA256(fullRunDigest || UTF8('/document/' || zeroBasedDocumentOrdinal)))":
        raise ValueError("v6 identity derivation changed")
    ...
    if workload.get("readersPerScenario") != 64 or workload.get("documentsPerScenario") != 8 or workload.get("lemmasPerDocument") != 400 or workload.get("scenarios") != ["well-specified", "item-residual", "response-noise", "idiosyncratic-knowledge"] or workload.get("modes") != ["all-unknown", "coverage-98"] or workload.get("warmAndCold") is not True:
        raise ValueError("v6 workload changed")

    runs = manifest.get("runs")
    if not isinstance(runs, list) or len(runs) != 3:
        raise ValueError("v6 must contain exactly three runs")
    seeds: set[int] = set()
    ...
    for ordinal, run in enumerate(runs):
        ...
        if run.get("runOrdinal") != ordinal or run.get("runID") != run_id or run.get("seed") != seed:
            raise ValueError(f"v6 run {ordinal} identity changed")
        if run.get("opaqueDocumentDerivationIDs") != expected_documents:
            raise ValueError(f"v6 run {ordinal} document identities changed")
        seeds.add(seed)
        documents.update(expected_documents)
    if len(seeds) != 3 or len(documents) != 24:
        raise ValueError("duplicate v6 identity")
    ...
    if file_count < 100 or not {1, 2, 7, 17, 20260909, 20260912, 20260913, 20260914}.issubset(prior_seeds):
        raise ValueError("v6 prior identity corpus is incomplete")
    ...
    if generator.get("path") != SOURCE_LOCK.relative_to(ROOT).as_posix() or generator.get("sha256") != SOURCE_LOCK_SHA256:
        raise ValueError("v6 generator lock binding changed")
    if verify_files:
        lock_bytes = SOURCE_LOCK.read_bytes()
        if sha256(lock_bytes) != SOURCE_LOCK_SHA256:
            raise ValueError("v6 generator lock bytes changed")
    ...
    if any(freeze.get(key) is not None for key in ("candidateFreeze", "analysisFreeze", "decisionRulesChecksum", "consumedByCandidateCycle")):
        raise ValueError("v6 prematurely claims a candidate/analysis freeze or consumption")
    if freeze.get("outcomeArtifacts") != [] or freeze.get("outcomesInspected") is not False or freeze.get("releaseHoldoutAccessAllowed") is not False:
        raise ValueError("v6 contains or authorizes protected outcomes")
    if freeze.get("requiredBeforeExecution") != ["candidate commit and clean-tree hash", "model/configuration/resource hashes", "analysis and decision-rule checksum", "reviewed decision to consume this specific reservation"]:
        raise ValueError("v6 execution prerequisites changed")
...
def self_test() -> None:
    ...
    for name, path in HISTORICAL_MANIFESTS.items():
        ...
        if not historical_seeds.issubset(prior_seeds) or not historical_documents.issubset(prior_documents):
            raise AssertionError(f"v6 prior identity corpus omitted {name} identities")
    ...
    for seed_set, document_set in ((prior_seeds | {first_seed}, prior_documents), (prior_seeds, prior_documents | {first_document})):
        try:
            ...
            raise AssertionError("accepted a synthetic v6 prior-identity collision")
    ...
    try:
        ...
        raise AssertionError("accepted mismatched historical v6 generator inputs")

    real_read_bytes = Path.read_bytes
    def missing_lock(path: Path) -> bytes:
        if path == SOURCE_LOCK:
            raise FileNotFoundError("synthetic missing v6 source lock")
    ...
    with patch.object(Path, "read_bytes", missing_lock):
        try:
            ...
            raise AssertionError("accepted missing v6 source lock")
    print("OtherWord-POS development-confirmation reservation v6 self-test passed")
...
def main() -> None:
    ...
    if sha256(MANIFEST.read_bytes()) != RESERVATION_SHA256:
        raise ValueError("v6 reservation is missing or differs from its frozen checksum")
    if args.self_test:
        self_test()
    else:
