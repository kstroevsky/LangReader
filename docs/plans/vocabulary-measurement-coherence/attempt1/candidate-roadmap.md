...
- Split the current `errorFloor` role into two explicit parameter families:
  ...
  - Use `P(K_j | θ, b_j) = εknowledge + (1 - 2 × εknowledge) × sigmoid(θ - b_j)` for latent knowledge.
...
- Make the offline fitter use the same categorical observation likelihood as production: fit `P(Eij | θi, bj, ρE)` rather than mapping evidence to hard or fractional target values. Keep the runtime evidence coefficients as fixed versioned priors for exploratory fitting; estimate them only from independently criterion-scored pilot data. Report which reliability source/version produced every calibration pack.
...
- Separate reader-serving item selection from calibration-data collection:
  ...
  - Freeze the slot rate, assignment policy, and inclusion probabilities before confirmatory collection. Do not tune them on the confirmatory holdout.
...
- Recruit multiple English/German learner, L1, and proficiency strata and multiple genres/formats. Freeze an untouched confirmatory set. The final split must be disjoint by participant and opaque study document ID; near-duplicate documents cannot cross the split.

- Use complete lexical audits for small documents. For large documents, test every selected card plus a probability/frequency/POS/occurrence-stratified sample of unselected items, including both high-confidence tails. Store each item’s audit inclusion probability and use weighted estimates with participant-clustered uncertainty intervals. Add a delayed stability/retention retest with randomized order and alternate context-free prompts.
...
- Fit and tune only on training/validation participants and documents. Evaluate once on the untouched participant-and-document holdout. Compare frequency-only, generic Rasch, warm-prior Rasch, and empirical-item variants plus simple baselines such as a frequency threshold and selecting all unranked words. Freeze numerical product gates before confirmatory data collection; synthetic gates remain separately labeled engineering diagnostics.
...
- Keep the current failing three-seed/eight-document version-3 release holdout frozen as adverse evidence. Do not weaken its gates or replace the version-2 golden with a favorable development result. Add a preregistered diagnostic matrix that varies, one factor at a time and then in bounded combinations:
...
- Extend calibration-tool tests with categorical evidence-likelihood fixtures, duplicate/selection-propensity checks, participant-and-document split leakage negatives, uniform/non-uniform DIF fixtures, anchor-purification convergence, multiple-comparison control, and exact pack provenance.
...
- Stage posterior-predictive computation before changing sample counts:
  ...
  - accept staged screening only if deterministic parity tests show that stop reason, selected deck, conservative coverage decision, and release-gate outcome stay within a preregistered tolerance;
...
- Do not reduce the cold ceiling merely because 80 questions feels long. After the observation model is coherent, evaluate explicit burden/uncertainty modes:
  ...
  Formalize stopping as `expected reduction in decision loss - λ × question cost`. Freeze each mode’s `λ`, maximum burden, displayed uncertainty language, and coverage-risk gate before confirmatory evaluation. Keep the current 20-question cold minimum and 80-question maximum until a replacement mode passes the real and synthetic gates.

## Deferred lexical and risk extensions

- Add high-value multi-word lexical items before enabling same-POS sense clustering. Candidate classes include phrasal verbs, idioms, lexical phrases, and compounds whose meaning is not recoverable from independently known component words. Define book occurrence identity, dictionary/reveal behavior, overlap resolution with single-word cards, and English/German labeled gates before production use.
