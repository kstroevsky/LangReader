...
This is an additive implementation plan for the reviewed validation improvements. It is not a replacement canonical roadmap, a release approval, an approved SAP, or permission to collect confirmatory data. The current task revises this plan and its verification/ledger artifacts and prepares detailed implementation instructions only. The latest user instruction stops this task before code changes. The first future execution milestone is synthetic causal diagnostics and its direct dependencies, ending at the checkpoint below for review.
...
Bank A remains the exact production 512-world bank used to construct the deck. Freeze the assessment posterior, epsilon, evidence, inventory, denominator and selected set before evaluating B1 or B2. Neither evaluation bank may affect selection, stopping, posterior updates, the reported production result or subsequent answers. References to B below mean the separately identified B1 and B2 families; never pool them into a single unexplained estimate.

- **B1:** use the same deterministic theta strata/posterior positions as production A, with independent latent-item draws. Preserve the production theta-stratum weights; if a larger bank repeats these positions, freeze and report the replication/weighting scheme. This isolates Bernoulli/latent-item mask-bank effects while holding the theta approximation fixed.
- **B2:** use independently randomized or shifted stratified posterior positions, plus independent latent-item draws. Freeze the position-generation method and seed derivation before outcomes. This probes the broader finite predictive approximation, including theta-position effects. B2 must not merely reuse A's deterministic theta positions or B1's masks.

Production A remains unchanged in both comparisons. Retain separate fingerprints for theta positions and latent-item draw streams for A, B1 and B2. Independence describes the randomized dimensions: B1 deliberately shares A's deterministic theta strata.

Implement a narrow, immutable Core diagnostic sampling/evaluation seam near the existing `predictiveCoverageSamples` owner. Share the existing knowledge and observation semantics instead of independently rewriting the likelihood in Python. Parameterize independent seed material, evaluation sample count and the explicit B1/B2 sampling mode only for the diagnostic call. Preserve existing production draws and mask ordering exactly. A copied assessment or immutable snapshot must not mutate production caches; do not call `result()` through a path that silently rebuilds a different deck.

Use independently derived deterministic streams for every evaluation replicate. The user's suggested 4,096 or 16,384 worlds are candidate diagnostic sizes, not new production defaults. Freeze sample count, replicate count, seeds and precision/convergence criteria in the development run manifest before inspecting outcomes. A latent-item seed change must affect that draw stream without changing B1 theta positions; a B2 position seed change must affect its randomized/shifted posterior positions independently of latent-item seeds. Deterministic B1 theta stratification must be described honestly rather than claiming every bank component is randomized. Repeated copies of A or its masks are not independent banks.

For fixed D report Q0.05(A), Q0.05(B1) and Q0.05(B2) for each replicate, their differences, estimated probability of coverage below 0.98 under each evaluation family, and numerical variability. Compare A/B1 for latent-item bank effects and B1/B2 for sensitivity to the broader theta-position approximation, retaining the uncertainty of each comparison; neither contrast alone proves a causal mechanism. Evaluate successes as well as misses to avoid conditioning the diagnosis on failure alone. Include decks fixed independently of A and small analytically tractable cases as controls. Separate posterior-model uncertainty, finite-bank evaluation uncertainty and variation across synthetic learners/documents.
...
Keep streams for document construction, item residuals, learner knowledge, response error, history/drift, predictive A, B1 latent-item draws, B2 posterior positions, B2 latent-item draws, and diagnostic success sampling separate. Serial and supported parallel runs must agree; isolate temporary stores and outputs. Do not modify a learner's hidden state merely because one variant asked a different number of questions unless a separately declared learning scenario explicitly models that effect.

Maintain three distinct synthetic data roles in that order: freely reusable diagnostic-development data; a fresh **development-confirmation** seed/document set; and the unchanged frozen release holdout. Reserve the development-confirmation manifest separately, with seed/document identities or immutable generation provenance and non-overlap checks. Do not use or inspect its outcomes during iterative design. Freeze the diagnostic/model candidate, configuration, analysis and decision rules before its first development-confirmation evaluation, and record that freeze with source/artifact hashes. Use this intermediate set before any release-holdout access. If its results motivate design changes, return to diagnostic-development data, retain the failed/null result and treat that set as consumed for that candidate-design cycle; a subsequent frozen candidate needs a fresh development-confirmation set. This adds a supplementary confirmation stage without changing release gates, holdout contents or the version-2 golden. Reserving manifests is allowed in the first milestone; accessing development-confirmation outcomes or the release holdout is outside that milestone.
...
Design rehearsals must report minimum inclusion probability, maximum design weight, effective sample size, and expected selected-card/final-tail support, with the frame, weighting convention, effective-sample-size definition and relevant participant/document/item grouping explicit. Examine sparse or empty selected-card/tail cells and sensitivity to unequal weights; do not confuse a weight-based effective sample size with independent participant/document support. These diagnostics inform the eventual SAP/design freeze. This plan supplies no numerical thresholds, trimming rules or estimator defaults for them.

Explicitly study **pretest interference** in consenting developmental work: prior retrieval attempts can change later self-verification, fatigue or learning even if labels remain hidden. Predeclare a feasible comparison/order design, record exposure and timing, and keep its data separate from confirmation. The amendment removes sampling circularity; it does not prove noninterference. **Pretest interference is an explicit blocking decision for confirmatory human collection:** confirmation must not proceed until evidence supports the chosen pretest procedure or the reviewed study design explicitly accounts for interference. Record the supporting evidence or design treatment, responsible review and freeze decision before confirmatory collection. Developmental studies may measure interference without satisfying this confirmation gate.
...
The two trained bilingual raters may access the hidden document-relevant sense/context in a restricted, consented scoring workspace, while remaining blind to model probabilities, deck decisions, warm/cold condition and each other's initial scores. Raw context and typed responses must not leak into the ordinary product export or public analysis package. For confirmatory criterion scoring, the target-sense/context mapping and scoring rubric must be frozen before confirmatory responses or model outcomes are inspected. There is no feasibility exception. Developmental rubric refinement must remain outside the confirmatory scoring material; ambiguities discovered later follow the frozen ambiguity/adjudication procedure and recorded amendment rules, rather than outcome-informed rewriting.
...
Sampling-design rehearsal outputs include the minimum inclusion probability, maximum design weight, effective sample size and expected selected-card/final-tail support specified above, alongside realized support. Sparse support and extreme weights must remain visible as design findings for the SAP; this rehearsal does not choose numerical acceptance thresholds.
...
**Exit condition:** every proposed primary endpoint has an explicit input map and executes with independently checked expected behavior, or is visibly blocked by a documented decision. Confirmation remains blocked until primary endpoint gaps and the pretest-interference decision are resolved under the approved SAP/design; the fabricated rehearsal is not real-learner evidence.
...
A consenting developmental pilot may inform protocol feasibility, rubric behavior, pretest interference, timing and precision assumptions. It must be labeled developmental before collection and never promoted into the untouched confirmatory sample. SAP, protocol, scoring rules, analysis code/configuration, outcome gates and analysis populations must be reviewed/frozen before confirmatory data collection and inspection. The freeze must explicitly close the pretest-interference blocking decision through supporting evidence or a design that accounts for interference. Any post-freeze amendment retains its rationale, authorization and original analysis; no outcome-driven estimator choice.

## Human comparison and learning outcomes

<!-- requirement: STUDY-VALUE -->

Separate the **core human validity study** from the **broader equal-total-preparation-time product-value comparison**. Core validity establishes item calibration, deck validity, realized coverage, burden and warm non-inferiority under its approved design. The broader product-value comparison remains valuable planned work, but does not automatically block those core claims or their study completion unless a specifically identified authoritative requirement makes it a release prerequisite. Trace any such prerequisite to its canonical requirement ID before applying it; this companion neither invents a new release gate nor removes an existing one. Verified learning and delayed retention needed by a core endpoint remain required by that endpoint, even if the broader comparison is scheduled separately.

For the broader comparison, prioritize the simple baselines already named in the pilot: frequency threshold, select-all-unranked, frequency-only/generic Rasch, accumulated warm Rasch, and empirical-item variants only when permitted by their gates. The existing personal no-test blind audit can support exploratory within-reader diagnosis, but its self-report is not the independent study criterion and its answers must not train the reader prior or become confirmatory labels.
...
| Pretest interference | Evidence supporting the chosen pretest procedure or a reviewed design explicitly accounting for interference | Blocking decision before confirmatory human collection; developmental measurement is allowed |
...
| Meaning prompt/ambiguity | Multiple context-free meanings, hidden target context for blinded raters, separate ambiguity status | Freeze target-sense/context mapping and scoring rubric before inspecting confirmatory responses or model outcomes, with no feasibility exception; freeze any consent/schema delta before real use; fabricated scoring examples can proceed |
...
| Estimation and precision | Named estimator/variance/missingness/multiplicity/design with justified participant/document support, minimum inclusion probability, maximum weight, effective sample size and expected selected-card/final-tail support; no numerical thresholds supplied here | Separately reviewed actual SAP before confirmatory collection or outcome inspection; provisional rehearsal methods clearly labeled |
...
| Diagnostic run design | Development seeds, distinct B1/B2 modes and sample sizes/replicates, budgets, coupling and materiality/precision rules | Freeze run manifest before viewing its outcomes; routine reversible diagnostic implementation needs no new canonical approval |
| Synthetic data access | Diagnostic-development, untouched development-confirmation, then frozen release holdout | Freeze candidate/analysis/decision rules before development-confirmation access; no iterative use; confirmation precedes release holdout; retain and retire consumed confirmation sets after design changes |
...
Core validity and the broader equal-time value comparison have separate completion decisions; only an identified authoritative release prerequisite can make the latter block core validity.

Prepare reviewable deltas against the accepted files, with affected requirement IDs, exact replacement text, compatibility/privacy effects and unchanged requirements. Do not rewrite the entire roadmap or call this companion canonical. A later explicit acceptance may authorize those deltas; elapsed time, generated files and an old approval record are not substitute approval. The latest user instruction limits the present task to plan revision and implementation handoff instructions, stopping before code changes. The future first synthetic causal-diagnostics milestone must stop at its review checkpoint before beginning subsequent slices.
...
| Forensic banks | Core/evaluation engineer: frozen-deck diagnostic seam, independent replicated banks, controls and missed-mass reports | Foundation | Reproducible A/B1/B2 diagnostic report with uncertainty and no runtime behavior change |
...
| Broader product-value comparison | Study owner: equal-total-preparation-time baselines and ordinary-use comparison | Separate design/SAP; not an automatic prerequisite for core human validity absent a cited authoritative requirement | Independently interpretable value/burden report; no inferred new release gate |
...
**First future execution boundary (not code authorization for this documentation task):** implement Foundation and Forensic banks as the first synthetic causal-diagnostics milestone, including natural/fixed-budget/common-replay counterfactual infrastructure only where it is a direct dependency. The milestone includes failed-deck records, exact occurrence-mass conservation, immutable A/B1/B2 frozen-deck evaluation, reproducibility/RNG-stream controls, fixed-bank and small-enumeration oracle fixtures. Do not implement study-data schema changes, production selector changes, warm-production changes, POS policy changes, calibration slots, or release-holdout tuning. Subsequent slices remain planned work, not authorization to execute them during this milestone.

At its checkpoint, stop for review and report changed files; mathematical/statistical invariants; proof that production outputs are unchanged with diagnostics off; deterministic/reproducibility tests; narrow test results; diagnostic overhead; discovered design issues; and a reasoned recommendation on proceeding to the longitudinal-warm slice. A recommendation is not permission to begin that slice.
...
3. Evaluate the frozen candidate first on the fresh, previously untouched development-confirmation seed/document set under its frozen analysis/decision rules. Do not inspect that set during iterative design. If it motivates changes, retain the result, return to diagnostic-development work and reserve a fresh confirmation set for the next frozen candidate. Run the complete evaluator and the frozen release holdout only after candidate freeze and the separate development-confirmation decision permits progression, preserving its original seed/population/gate definitions. New development suites are supplementary. Retain failed candidates and return to development if any gate fails; do not keep querying the same holdout while changing parameters to make it green.
...
6. Confirm required real-learner validation has been executed under the frozen protocol/SAP, with the pretest-interference blocking decision resolved and target-sense/context mapping plus rubric frozen before confirmatory outcome inspection, before making its gated claims. A successful fabricated rehearsal or synthetic suite cannot substitute. Any unfinished required item keeps the appropriate release/claim status blocked; distinguish readiness to collect data from readiness to release. Keep broader equal-time product-value work separate from core validity unless an identified authoritative requirement makes it a release prerequisite.
...
- [ ] Frozen-deck A/B1/B2 replicas, separate theta-position and latent-item controls, fixed-deck controls and convergence evidence distinguish sampling variability from systematic optimism; no evaluation-bank decision leakage.
- [ ] Fresh development-confirmation seeds/documents remain unused during iterative design; freeze precedes confirmation access, and the confirmation decision precedes release-holdout access.
...
- [ ] Large-document sampling amendment is concrete; pretest interference blocks confirmatory collection until supported or accounted for in the reviewed design; small-document census, positive inclusion probabilities and pre-reveal timing are preserved.
...
- [ ] Multiple-meaning rubric, rater blinding, ambiguity and adjudication have executable examples and consent-compatible access boundaries; target-sense/context mapping and rubric freeze before confirmatory response/model-outcome inspection without a feasibility exception.
...
- [ ] SAP precision/sample-size planning, human theta endpoint decision, within-document coverage uncertainty and warm comparison design are reviewed before confirmation; design diagnostics cover minimum inclusion probability, maximum weight, effective sample size and expected selected-card/final-tail support without plan-invented thresholds.
- [ ] Developmental human work stays separate; core validity and broader equal-time product-value comparison have separate completion decisions, with the latter blocking only through an identified authoritative prerequisite; retain baselines, verified immediate learning and delayed retention without conflating projection or comprehension.
...
The first future milestone ends at the synthetic causal-diagnostics checkpoint in Delivery sequence and ownership; this whole-plan checklist does not authorize later slices. The present documentation task stops before writing implementation code.
