...
- Split the current `errorFloor` role into two explicit parameter families:
  ...
  - **Authorized mathematical change** from `authorization-2026-08-25.md` (SHA-256 `762b97c9fff039b46f13d235a9e69b0eec1f37e7dcf29c7e7b1d9bf9a46f53f1`): in the revised observation model, remove `min(reliability, 1 - errorFloor)` from both current source symbols `evidenceLikelihood` and `posteriorKnownProbability`. Evidence-specific `ρ` values are used directly and never capped by `εknowledge`.
  - Do not flatten latent knowledge and cap response reliability with one shared parameter.
  - For latent known probability `p`, evidence supporting known has likelihood `ρE × p + (1 - ρE) × (1 - p)`; evidence supporting unknown has likelihood `ρE × (1 - p) + (1 - ρE) × p`. Answered-item Bayes reconstruction uses this same likelihood implementation rather than a second reliability rule.
  - Tail-validation contradictions update only `εknowledge`. They do not automatically reduce any evidence-specific `ρ`. Future independently scored evidence that a response protocol is noisy updates the corresponding `ρ`, not `εknowledge`.
...
- Make a versioned Core `VocabularyObservationModel.evidenceLikelihood(evidence, latentKnownProbability)` the single normative implementation of the newly authorized categorical observation likelihood. The hash-locked `AdaptiveVocabularyAssessment.swift` symbols `evidenceLikelihood` and `posteriorKnownProbability` are pre-change provenance, not a requirement to preserve their shared cap. Runtime posterior updates and answered-item reconstruction call the new Core owner directly. The offline fitter consumes a generated observation-model manifest plus golden likelihood fixtures produced from that same Core implementation; it must not maintain an independently authored hard/fractional mapping or formula.
...
- Use complete lexical audits for small documents. For large documents, test every selected card plus a probability/frequency/POS/occurrence-stratified sample of unselected items. Store each item’s audit inclusion probability, preserve participant/document clustering, and require weighted estimation appropriate to the sampling design. Add a delayed stability/retention retest. This roadmap deliberately does not choose the weighting estimator or variance/interval construction.
...
- **Statistical Analysis Plan gate**, authorized by the same hash-locked authorization artifact: before confirmatory data collection begins, a separately reviewed and approved SAP must freeze the estimand, weighting estimator, treatment of inclusion probabilities, participant/document clustering structure, variance/interval method, missing-data rules, multiplicity policy where applicable, confirmatory outcomes and acceptance gates, and confirmatory analysis populations. The SAP must be approved before inspecting confirmatory outcomes; estimator or interval choices must not be selected retrospectively to improve the observed result. The roadmap does not select Horvitz–Thompson versus Hájek estimation, bootstrap versus sandwich versus design-based intervals, or another method.
